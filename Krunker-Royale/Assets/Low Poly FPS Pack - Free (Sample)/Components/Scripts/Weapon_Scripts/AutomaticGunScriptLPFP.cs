...
public class AutomaticGunScriptLPFP : NetworkBehaviour {
	...
	[SerializeField] public FpsControllerLPFP player;
	...
	private void Start() {
		...
		//Set total ammo text from tota	l ammo int
		...
		if (!chatinput.isFocused)
		{
			...
			if (Input.GetMouseButton(0) && !outOfAmmo && !isReloading && !isInspecting && !isRunning)
			{
				...
				if (Time.time - lastFired > 1 / fireRate)
				{
					...
					NetworkServer.Spawn(bullet.gameObject, player.connectionToClient);
    ...
    {
        player.displayName = "cmdplayerShoot";
        RpcPlayerShoots();

    }

    [ClientRpc]
    private void RpcPlayerShoots()
    {
        player.displayName = "RpcplayerShoot";
        //Spawn bullet from bullet spawnpoint
        var bullet = (Transform)Instantiate(
            Prefabs.bulletPrefab,
            Spawnpoints.bulletSpawnPoint.transform.position,
