...
#pragma warning restore 649
        ...
        public override void OnStartClient(){
            ...
            displayName = manager.playerNamesList[manager.playerNamesList.Count - 1];
            manager.playerNamesList.RemoveAt(manager.playerNamesList.Count - 1);
        }

        public override void OnStartAuthority()
        {
            playerArms.SetActive(true);
            soldierBody.SetActive(false);
            minimapIcon.SetActive(true);
            _networkIdentity = GetComponent<NetworkIdentity>();
            //uint xd = _networkIdentity.netId;
            //GameObject zonewall = GameObject.Find("/Safezone/ZoneWall");
        }
        /// Initializes the FpsController on start.
        private void Start()
        {
            //GameObject armsToRender = GameObject.Find("/Player(Clone)/arms_assault_rifle_01");
            //armsToRender.GetComponent<Renderer>().enabled = false;
        ...
        {
            if (!isLocalPlayer) return;
            if (Input.GetKeyDown(KeyCode.C)) { chatInput.ActivateInputField(); }
            //if (Input.GetKeyDown(KeyCode.Return)) { chatBoxOpen = false; chatInput.SendMessage("Send", "XD"); }
